...
public record FacilityCreateRequest (
        @NotBlank(message = FacilityValidationMessage.FACILITY_CODE_REQUIRED)
        @Size(max=20, message = FacilityValidationMessage.FACILITY_CODE_MAX_LENGTH)
        @Pattern(regexp = "^[A-Z0-9]{3,20}$", message = FacilityValidationMessage.FACILITY_CODE_PATTERN)
        String facilityCode,
